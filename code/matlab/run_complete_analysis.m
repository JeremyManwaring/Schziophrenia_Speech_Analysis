...
function preprocess_subject(sub_id)
    ...
    % Segmentation - uses original anatomical (coregistration updates header in-place)
    ...
    % Normalization (will be set up after segmentation completes)
    % Deformation field is named after original anatomical (not rsub-*)
    ...
    % Initialize normalization batch (will update file paths after segmentation)
    matlabbatch{3}.spm.spatial.normalise.write.subj.def = {deformation_field};
    matlabbatch{3}.spm.spatial.normalise.write.subj.resample = {''}; % Will be updated
    ...
    matlabbatch{4}.spm.spatial.smooth.data = {''}; % Will be updated
    ...
    % Run segmentation (uses original anatomical with updated header from coregistration)
    fprintf('    Running segmentation...\n');
    spm_jobman('run', matlabbatch(2));
    
    % Wait for segmentation and deformation field (named after original anatomical)
    deformation_field = fullfile(anat_dir, sprintf('y_sub-%s_T1w.nii', sub_id));
    wait_time = 0;
