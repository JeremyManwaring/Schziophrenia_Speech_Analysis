"""Build the complete manuscript figure set in an editorial neuroscience style.

The script is rendering-only. It reads the stored statistical records and
participant-level values, does not rerun any model, and writes a separate set
to ``results/paper_figures_nature`` so all prior figure exports remain intact.
"""
...
from nature_figure_style import (  # noqa: E402
    AXIS_COLOR,
    COOL_PAPER,
    CORAL,
    DIVERGING_CMAP,
...
OUTPUT_DIR = BASE_DIR / "results" / "paper_figures_nature"
...
def figure_1_core_results() -> None:
    ...
    fig = plt.figure(figsize=(FIGURE_WIDTH, 6.15))
    fig.text(
        0.5,
        0.972,
        "Main results",
    ...
    fig.text(
        0.5,
        0.942,
        "Adjusted findings with participant-level context",
        ha="center",
        va="top",
        fontsize=6.5,
        color=MUTED_INK,
    ...
    ax.set_xlabel("Adjusted effect size, Cohen's d")
    ax.text(
        0.015,
        0.035,
        "Higher in AVH+",
    ...
    panel_header(
        ...
        "Cohen's d with 95% CI · Sentences vs Reversed",
    )
    ...
    ax.set_xlabel("Age/IQ-adjusted activation (beta)")
    ax.set_ylabel("Age/IQ-adjusted PSYRATS score")
    panel_header(
        ax,
        "B",
        "Symptom association",
        f"AVH+ · {format_roi(hit['roi'])} · {format_contrast(hit['contrast'])} · n = {hit['n']}",
    )
    ...
    panel_header(
        ax,
        "C",
        "Participant-level activation",
        "Raw regional values · Sentences vs Reversed",
        header_y=1.17,
    )
    style_axis(ax, grid_axis="y")
    clean_axis(ax)
    ...
    details = [
        (
            0.09,
            "A · Adjusted effects",
            "ANCOVA adjusted for age, IQ, sex and mean FD.\n"
            "95% CI; Bonferroni-adjusted p; full n = 69.",
        ),
        (
            0.395,
            "B · Symptom association",
            "Age/IQ-residualized values; band shows 95% CI.\n"
            f"AVH+ n = {hit['n']}; df = {hit['df']}.",
        ),
        (
            0.70,
            "C · Participant context",
            "Raw beta values; violins show density.\n"
            "Boxes show median/IQR; points are participants.",
        ),
    ]
    for x, heading, text in details:
        fig.text(
            x,
            0.116,
            heading,
...
def figure_2_effect_size_landscape() -> None:
    ...
    fig = plt.figure(figsize=(FIGURE_WIDTH, 4.55))
    ax = fig.add_axes([0.205, 0.285, 0.755, 0.515])
    cbar_ax = fig.add_axes([0.335, 0.145, 0.40, 0.018])
    ...
    # Quietly organize task contrasts, baseline contrasts, and hemispheres.
    ...
    fig.text(
        0.5,
        0.925,
        "Standardized AVH− versus AVH+ differences across 12 ROIs and 7 task contrasts",
        ha="center",
        va="top",
        fontsize=6.45,
        color=MUTED_INK,
...
def figure_3_roi_definitions() -> None:
    fig = plt.figure(figsize=(FIGURE_WIDTH, 4.85))
    figure_title(
        fig,
        "Anatomical ROI definitions",
        "Twelve spherical regions in MNI space",
        x=0.055,
        y=0.965,
    )
    ...
    fig.text(
        0.082,
        0.843,
        "Representative bilateral sagittal, coronal and axial cuts",
        ha="left",
        va="bottom",
        fontsize=6.1,
        color=MUTED_INK,
    ...
    for index, (label, mode, coordinate) in enumerate(slice_specs):
        left = map_left + index * (map_width + map_gap)
        ax_map = fig.add_axes([left, 0.565, map_width, 0.215])
        display = plot_anat(
            template,
            display_mode=mode,
    ...
    left_rows = [
        [
            _roi_display(key),
            f"{ROIS[key][0]:.0f}, {ROIS[key][1]:.0f}, {ROIS[key][2]:.0f}",
            f"{ROI_RADII[key]} mm",
        ]
    ...
    ax_left = fig.add_axes([0.055, 0.105, 0.42, 0.285])
    ax_right = fig.add_axes([0.525, 0.105, 0.42, 0.285])
